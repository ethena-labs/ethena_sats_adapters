...
from constants.euler import BERA_MEV_SUSDE_URL
...
class EulerBeraMevSUSDEIntegration(CachedBalancesIntegration):
    def __init__(self):
        super().__init__(
            integration_id=IntID.EULER_BERA_MEV_SUSDE,
            start_block=2772242,
            chain=Chain.BERACHAIN,
            summary_cols=[SummaryColumn.EULER_BERA_MEV_SUSDE_PTS],
            reward_multiplier=5,
    ...
    def get_data_for_block(self, block: int) -> Dict[ChecksumAddress, float]:
        if block < self.start_block:
            return {}
        url = f"{BERA_MEV_SUSDE_URL}?blockNumber={block}"
