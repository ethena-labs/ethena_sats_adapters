...
class StrataSrUSDeIntegration(CachedBalancesIntegration):
    def __init__(
        self,
        integration_id: IntegrationID = IntegrationID.STRATA_MONEY_SENIOR,
        chain: Chain = Chain.ETHEREUM,
        summary_cols: Optional[List[SummaryColumn]] = [SummaryColumn.STRATA_MONEY_SENIOR],
        ...
        # Initialize tranche
        tranche = next((c for c in strata["tranches"] if c["name"] == "srUSDe"), None)
        if tranche is None:
            raise ValueError("srUSDe tranche not found in strata config")
...
if __name__ == "__main__":
    example_integration = StrataSrUSDeIntegration()
    ...
    integration_1 = StrataSrUSDeIntegration()
    balances = integration_1.get_block_balances(
        cached_data={}, blocks=[ BLOCK_2 ]
    )

    user = Web3.to_checksum_address("0xC69c87351Bf66cAB8E5676D6f2D4b63299Ad0835")
