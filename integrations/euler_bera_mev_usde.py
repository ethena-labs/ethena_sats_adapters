...
from constants.euler import BERA_MEV_USDE_URL
...
class EulerBeraMevUSDEIntegration(CachedBalancesIntegration):
    def __init__(self):
        super().__init__(
            integration_id=IntID.EULER_BERA_MEV_USDE,
            start_block=2772190,
            chain=Chain.BERACHAIN,
            summary_cols=[SummaryColumn.EULER_BERA_MEV_USDE_PTS],
    ...
    def get_data_for_block(self, block: int) -> Dict[ChecksumAddress, float]:
        if block < self.start_block:
            return {}
        url = f"{BERA_MEV_USDE_URL}?blockNumber={block}"
