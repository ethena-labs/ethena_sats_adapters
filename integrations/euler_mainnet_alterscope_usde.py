...
from constants.euler import MAINNET_ALTERSCOPE_USDE_URL
...
class EulerMainnetAlterscopeUSDEIntegration(CachedBalancesIntegration):
    def __init__(self):
        super().__init__(
            integration_id=IntID.EULER_MAINNET_ALTERSCOPE_USDE,
            start_block=21033816,
            chain=Chain.ETHEREUM,
            summary_cols=[SummaryColumn.EULER_MAINNET_ALTERSCOPE_USDE_PTS],
    ...
    def get_data_for_block(self, block: int) -> Dict[ChecksumAddress, float]:
        if block < self.start_block:
            return {}
        url = f"{MAINNET_ALTERSCOPE_USDE_URL}?blockNumber={block}"
