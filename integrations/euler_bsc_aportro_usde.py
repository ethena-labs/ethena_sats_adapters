...
from constants.euler import BSC_APOSTRO_USDE_URL
...
class EulerBscAportroUSDEIntegration(CachedBalancesIntegration):
    def __init__(self):
        super().__init__(
            integration_id=IntID.EULER_BSC_APOSTRO_USDE,
            start_block=48173432,
            chain=Chain.BSC,
            summary_cols=[SummaryColumn.EULER_BSC_APOSTRO_USDE_PTS],
    ...
    def get_data_for_block(self, block: int) -> Dict[ChecksumAddress, float]:
        if block < self.start_block:
            return {}
        url = f"{BSC_APOSTRO_USDE_URL}?blockNumber={block}"
