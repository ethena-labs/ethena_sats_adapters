...
from constants.euler import MAINNET_MAXI_USDE_URL
...
class EulerMainnetMaxiUSDEIntegration(CachedBalancesIntegration):
    def __init__(self):
        super().__init__(
            integration_id=IntID.EULER_MAINNET_MAXI_USDE,
            start_block=20684152,
            chain=Chain.ETHEREUM,
            summary_cols=[SummaryColumn.EULER_MAINNET_MAXI_USDE_PTS],
    ...
    def get_data_for_block(self, block: int) -> Dict[ChecksumAddress, float]:
        if block < self.start_block:
            return {}
        url = f"{MAINNET_MAXI_USDE_URL}?blockNumber={block}"
