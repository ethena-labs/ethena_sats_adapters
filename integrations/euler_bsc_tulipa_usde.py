...
from constants.euler import BSC_TULIPA_USDE_URL
...
class EulerBscTulipaUSDEIntegration(CachedBalancesIntegration):
    def __init__(self):
        super().__init__(
            integration_id=IntID.EULER_BSC_TULIPA_USDE,
            start_block=48288923,
            chain=Chain.BSC,
            summary_cols=[SummaryColumn.EULER_BSC_TULIPA_USDE_PTS],
    ...
    def get_data_for_block(self, block: int) -> Dict[ChecksumAddress, float]:
        if block < self.start_block:
            return {}
        url = f"{BSC_TULIPA_USDE_URL}?blockNumber={block}"
