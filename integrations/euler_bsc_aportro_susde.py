...
from constants.euler import BSC_APOSTRO_SUSDE_URL
...
class EulerBscAportroSUSDEIntegration(CachedBalancesIntegration):
    def __init__(self):
        super().__init__(
            integration_id=IntID.EULER_BSC_APOSTRO_SUSDE,
            start_block=48432835,
            chain=Chain.BSC,
            summary_cols=[SummaryColumn.EULER_BSC_APOSTRO_SUSDE_PTS],
            reward_multiplier=5,
    ...
    def get_data_for_block(self, block: int) -> Dict[ChecksumAddress, float]:
        if block < self.start_block:
            return {}
        url = f"{BSC_APOSTRO_SUSDE_URL}?blockNumber={block}"
