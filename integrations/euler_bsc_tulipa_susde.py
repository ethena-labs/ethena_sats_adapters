...
from constants.euler import BSC_TULIPA_SUSDE_URL
...
class EulerBscTulipaSUSDEIntegration(CachedBalancesIntegration):
    def __init__(self):
        super().__init__(
            integration_id=IntID.EULER_BSC_TULIPA_SUSDE,
            start_block=48288924,
            chain=Chain.BSC,
            summary_cols=[SummaryColumn.EULER_BSC_TULIPA_SUSDE_PTS],
            reward_multiplier=5,
    ...
    def get_data_for_block(self, block: int) -> Dict[ChecksumAddress, float]:
        if block < self.start_block:
            return {}
        url = f"{BSC_TULIPA_SUSDE_URL}?blockNumber={block}"
