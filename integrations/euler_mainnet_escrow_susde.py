...
from constants.euler import MAINNET_ESCROW_SUSDE_URL
...
class EulerMainnetEscrowSUSDEIntegration(CachedBalancesIntegration):
    def __init__(self):
        super().__init__(
            integration_id=IntID.EULER_MAINNET_ESCROW_SUSDE,
            start_block=20690379,
            chain=Chain.ETHEREUM,
            summary_cols=[SummaryColumn.EULER_MAINNET_ESCROW_SUSDE_PTS],
            reward_multiplier=5,
    ...
    def get_data_for_block(self, block: int) -> Dict[ChecksumAddress, float]:
        if block < self.start_block:
            return {}
        url = f"{MAINNET_ESCROW_SUSDE_URL}?blockNumber={block}"
