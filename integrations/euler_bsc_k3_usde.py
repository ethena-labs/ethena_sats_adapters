...
from constants.euler import BSC_K3_USDE_URL
...
class EulerBscK3USDEIntegration(CachedBalancesIntegration):
    def __init__(self):
        super().__init__(
            integration_id=IntID.EULER_BSC_K3_USDE,
            start_block=48377339,
            chain=Chain.BSC,
            summary_cols=[SummaryColumn.EULER_BSC_K3_USDE_PTS],
    ...
    def get_data_for_block(self, block: int) -> Dict[ChecksumAddress, float]:
        if block < self.start_block:
            return {}
        url = f"{BSC_K3_USDE_URL}?blockNumber={block}"
