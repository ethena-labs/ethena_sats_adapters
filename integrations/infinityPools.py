...
from constants.infinityPools import START_BLOCK, usdc_sUSDe, wstETH_sUSDe
...
class InfinityPoolsIntegration(Integration):
    def __init__(
        self,
    ...
    def get_block_balances(
        ...
        pool_info_list = get_infinityPools_info_list(
            {usdc_sUSDe, wstETH_sUSDe}, start_block, end_block
        )
            ...
            for pool_info in pool_info_list:
                ...
                for user in pool_user_balance:
                    if user not in user_data:
                        user_data[user] = 0
                    user_data[user] = round(
                        pool_user_balance[user] / 10**18 + user_data[user], 4
                    )
...
if __name__ == "__main__":
    ...
    print(example_integration.get_block_balances(cached_data={}, blocks=[25101690]))
    # Example output:
