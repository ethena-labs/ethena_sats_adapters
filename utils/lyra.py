...
SUSDE_VAULT_ADDRESS = "0x0b4eD379da8eF4FCF06F697c5782CA7b4c3E505E"
...
def get_exchange_balance(user: str, block: int) -> int:
    user = user.lower()
    # Replace the user and block placeholders in the query
    user_balance_query_filled = user_balance_query.replace("{user}", user).replace("{block}", str(block))
    ...
    response = requests.post(url, json={'query': user_balance_query_filled}, headers=headers)
...
def get_effective_balance(user: str, block: int, integration_token: Contract, bridge: Contract, vault_token: Contract, timestamp):
    """
    ...
    # Send the POST request
    response = requests.post("https://api.lyra.finance/public/get_vault_share", json={'from_timestamp_sec': 0, 'to_timestamp_sec': timestamp, 'vault_name': 'sUSDePrincipal Protected Bull Call Spread', 'page_size': 1}, headers=headers)
    try:
        response_json = response.json()
        ...
        for balance in balances:
            if balance['subaccount']['matchingOwner'] is not None:
                if balance['subaccount']['matchingOwner']['owner'] is not None:
                    users.add(Web3.to_checksum_address(balance['subaccount']['matchingOwner']['owner']))
                else:
                    users.add(Web3.to_checksum_address(balance['subaccount']['matchingOwner']['id']))
            else:
                if balance['subaccount']['owner']['owner'] is not None:
                    users.add(Web3.to_checksum_address(balance['subaccount']['owner']['owner']))
                else:
                    users.add(Web3.to_checksum_address(balance['subaccount']['owner']['id']))
    else:
        print(f"Query failed with status code {response.status_code}: {response.text}")

    try:
        users.remove(SUSDE_VAULT_ADDRESS) # Ignore the sUSDe vault which is accounted for separately
    except:
        pass
