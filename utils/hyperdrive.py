...
def get_first_contract_block(contract_address):
    ...
    ), f"something fucked up since {earliest_block=} isn't greater than or equal to {latest_block=}"
...
def get_pool_details(pool_contract):
    ...
    config_outputs = pool_contract.functions.getPoolConfig().abi["outputs"][0][
        "components"
    ]
    ...
    if config["vaultSharesToken"] != "0x0000000000000000000000000000000000000000":
        vault_shares_contract = w3.eth.contract(
            address=config["vaultSharesToken"], abi=ERC20_ABI
        )
        vault_shares_balance = vault_shares_contract.functions.balanceOf(
...
def get_pool_positions(
    ...
    for user, id in itertools.product(pool_users, pool_ids):
        ...
        if bal > Decimal(1):
            pool_positions.append(
                [user, trade_type, prefix, timestamp, bal, Decimal(0)]
            )
    ...
    for prefixes in combined_prefixes:
        combined_shares = sum(
            position[5] for position in pool_positions if position[2] in prefixes
        )
        combined_rewardable = (
            lp_rewardable_tvl if prefixes[0] == 0 else short_rewardable_tvl
        )
        if combined_shares != combined_rewardable:
            ...
            max_position = max(
                (p for p in pool_positions if p[2] in prefixes), key=lambda x: x[5]
            )
            max_position[5] += diff
